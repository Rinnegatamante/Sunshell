...
no_preview = Screen.createImage(400,240,white)
preview_table = {}
max_previews = 16
b_previews = 1
...
while true do
	...
	if (Controls.check(pad,KEY_A)) and not (Controls.check(oldpad,KEY_A)) then
		...
		Screen.flip()
		Screen.waitVblankStart()
		ChangeTheme(System.currentDirectory()..themes_table[p_chmm].name)
	elseif (Controls.check(pad,KEY_DUP)) and not (Controls.check(oldpad,KEY_DUP)) then
			p_chmm = p_chmm - 1
			update = true
		if (p_chmm >= 16) then
			master_index_chmm = p_chmm - 15
		end
	...
	elseif (p_chmm > #themes_table) then
		master_index_chmm = 0
		p_chmm = 1
	end
	...
	while (p_chmm > max_previews) do
		max_previews = max_previews + 16
		b_previews = b_previews + 16
		UpdatePreviews()
	end
	while (p_chmm < b_previews) do
		max_previews = max_previews - 16
		b_previews = b_previews - 16
		UpdatePreviews()
	end
	if update then
		r_p_chmm = p_chmm
		while (r_p_chmm > 16) do
			r_p_chmm = r_p_chmm - 16
		end
		current_file = preview_table[r_p_chmm]
		big_image = false
		if Screen.getImageWidth(current_file) > 400 then
			width = 400
			big_image = true
